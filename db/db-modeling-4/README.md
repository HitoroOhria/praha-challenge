...
以下、長い&あまりまとまっていませんが Redis を導入する理由になります。
...
- Redis により高速化する
  - Redis は KVS であり、単一キー指定であればいかなる条件でも高速に値を取得できる特性を持つ
    - ここでいう条件とは、KVS のデータ件数やデータ量を指す
    - 部分一致取得や全件取得で指定する場合は、条件に応じて遅くなる
  - Redis を導入することにより、全体的な構成がシンプルになる
    - DB は事実となるデータを保存することに集中できる
    - Redis はアプリケーション処理高速化のために必要なデータを保存することに集中する
